...
export const C4BaseBox = (
  ...
  // ----- Box
  const box = createBox(data, options, customTagsStyle)
  objects.push(box)

  // ----- Tags
  let tags
  if (data.base_data?.tags) {
    tags = createTags(box, data, options, customTagsStyle)
    objects.push(tags)
  }

  // ----- Label
  const label = createLabel(box, tags, data, options, customTagsStyle)
  objects.push(label)

  // ----- Type / Technology
  const typeTechnology = createTypeTechnology(
    box,
    label,
    data,
    options,
  ...
  if (data.base_data?.link) {
    objects.push(createLinkIcon(box, options, customTagsStyle))
  }
...
const createLabel = (
  ...
  return new fabric.Text(unescape(data.base_data?.label) ?? 'undefined', {
    fill: customTagsStyle?.font_color ?? options.textColor,
    fontFamily: ELEMENT.FONT.FAMILY,
...
const createTypeTechnology = (
  ...
  return new fabric.Text(text, {
    ...
    left: (box.left ?? ELEMENT.SIZES.DEFAULT_LEFT) + (box.width ?? 0) / 2,
    top:
      (label?.top ?? ELEMENT.SIZES.DEFAULT_TOP) +
      BOX.SIZES.TOP_TYPE_TECHNOLOGY +
      (label?.height ?? 0),
  })
}
...
const createDescription = (
  ...
  return new fabric.Textbox(unescape(data.base_data?.description) ?? '', {
    fill: customTagsStyle?.font_color ?? options.textColor,
    fontFamily: ELEMENT.FONT.FAMILY,
