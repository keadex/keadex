...
const DEFAULT_POINTS = [
  new ExtendedPoint(
    0 + ELEMENT.SIZES.DEFAULT_LEFT,
    0 + ELEMENT.SIZES.DEFAULT_TOP,
    RelObjects.Dot
  ),
  // new ExtendedPoint(
  //   50 + ELEMENT.SIZES.DEFAULT_LEFT,
  //   0 + ELEMENT.SIZES.DEFAULT_TOP,
  //   RelObjects.Dot
  // ),
  new ExtendedPoint(
...
export const C4Relationship = (
  ...
  if (elementSpec) {
    ...
    if (savedDots && savedDots.length > 0)
      scale = savedDots[0].size?.scale_x ?? 1
  }
  ...
  const dotPoints = createPoints(elementSpec)
  const dots = createDots(undefined, dotPoints, elementSpec, parent)
...
// --------------- OBJECTS
const createPoints = (elementSpec?: DiagramElementSpec): ExtendedPoint[] => {
  ...
  if (elementSpec?.shapes) {
    ...
    points = points.concat(DEFAULT_POINTS)
  }
  return points
}
