...
export const C4Relationship = (
  ...
  // createBackground(elementSpec, dots, parent, undefined) @deprecated
...
const updateLinePoints = (line: fabric.Polyline) => {
  ...
  if (parent && canvas) {
    ...
    if (parent.children) {
      ...
      const oldSelectedColor = line.data?.otherData.selectedRelColor
      canvas.remove(line)
      ...
      // ------> Start Remove and re-create the background @deprecated
      ...
      // ------> End Remove and re-create the background
  ...
  if (event.pointer && canvas) {
    ...
    if (parent) {
      ...
      const newDot = createDot(
        ...
        dot.scaleY,
        line.stroke,
        dot.data?.otherData.selectedRelColor,
        false,
      )
...
const createDot = (
  ...
  if (isHeadArrow) {
    ...
    attachListenersToDot(headArrow, lineColor)
    return headArrow
  } else {
    const dot = new fabric.Circle({
      ...
      selectable: !autoLayoutEnabled,
      hasBorders: !autoLayoutEnabled,
    })
    dot.data = {
      rawDiagramElementSpec: point.data?.rawDiagramElementSpec,
      otherData: {
    ...
    attachListenersToDot(dot, lineColor)
    return dot
  }
}
...
const createLine = (
  ...
  line.parent = parent
  attachListenersToLine(line, line.stroke, line.strokeWidth)
  parent.children?.push(line)
  return line
}
...
const attachListenersToLine = (
  ...
  line.on(OBJECT_EVENTS.SELECTED, (e) => {
    line.stroke = line.data?.otherData?.selectedRelColor
    line.strokeWidth =
      (line.strokeWidth ?? 0) +
      RELATIONSHIP.SIZES.STROKE_SELECTED_INCREMENT_WIDTH
    line.dirty = true
    line.canvas?.renderAll()
    onRelationshipSelected(e, line)
  })
  line.on(OBJECT_EVENTS.DESELECTED, (e) => {
...
const attachListenersToDot = (dot: fabric.Object, lineColor?: string) => {
  ...
  dot.on(OBJECT_EVENTS.SELECTED, (e) => {
    dot.fill = dot.data?.otherData?.selectedRelColor
    dot.dirty = true
    dot.canvas?.renderAll()
  })
  dot.on(OBJECT_EVENTS.DESELECTED, (e) => {
    dot.fill = lineColor
    dot.dirty = true
    dot.canvas?.renderAll()
  })
