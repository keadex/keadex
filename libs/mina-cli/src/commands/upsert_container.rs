use crate::commands::{create_container::create_container, update_container::update_container};
...
pub async fn upsert_container(args: UpdateContainer) -> Result<(), MinaError> {
  let result = search_library_element(&args.alias).await;
  if let Ok(found) = result {
    if found.is_some() {
      // The element exists, so we can update it
      let result = update_container(args).await;
      if result.is_ok() {
  ...
  if args.new_label.is_some() && args.new_container_type.is_some() && args.new_technology.is_some()
  {
    ...
    let create_args = CreateContainer {
    ...
    let result = create_container(create_args).await;
    if result.is_ok() {
      let response = Response {
        code: 0,
