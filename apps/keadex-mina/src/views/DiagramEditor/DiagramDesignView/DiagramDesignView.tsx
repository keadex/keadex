...
export interface DiagramDesignViewProps {
  diagramDesignViewToolbarCommands?: RefObject<DiagramDesignViewToolbarCommands | null>
...
export interface DiagramDesignViewCommands {
  resetCanvas: (canvasState?: C4DiagramCanvasState) => void
  getUpdatedDiagramSpec: () => DiagramSpec | undefined
  getCanvasState: () => C4DiagramCanvasState
  exportDiagram: () => void
...
export const DiagramDesignView = forwardRef(
  (props: DiagramDesignViewProps, ref: Ref<DiagramDesignViewCommands>) => {
    ...
    const currentRenderedDiagram = useRef<Diagram | null>(undefined)
    ...
    function createCanvas(canvasState?: C4DiagramCanvasState) {
      ...
      if (canvasState) {
        c4DiagramRef.current?.setZoom(canvasState.zoom)
        c4DiagramRef.current?.setPan(canvasState.pan)
      }
    ...
    function resetCanvas(canvasState?: C4DiagramCanvasState) {
      destroyCanvas()
      createCanvas(canvasState)
    }

    function getCanvasState(): C4DiagramCanvasState {
      return {
        zoom: c4DiagramRef.current?.getZoom() ?? 1,
        pan: c4DiagramRef.current?.getPan() ?? { x: 0, y: 0 },
      }
    }
