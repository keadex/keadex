...
import { MinaReactClient } from '../../../components/client-wrappers'
...
There are three main differences between the Keadex Mina Rendering System and the rendering systems of other _diagrams as code_ tools:
...
The [Mina Frontend](/en/docs/mina/architecture/modules#frontend) invokes the Tauri commands exposed by the _Diagram Controller_ in the [Mina Backend](/en/docs/mina/architecture/modules#backend) to get the data of the diagram selected by the user in JSON format.

**[3] Controllers &rarr; Repositories**

The _Diagram Controller_ invokes the _Diagram Repository_ to read the code (JSON and PlantUML) of the diagram selected by the user.

**[4] Repositories &rarr; Resolver**

The _Diagram Repository_ retrieves the _Diagram Spec and PlantUML File System DAOs_ from the [resolver](/en/docs/mina/architecture/modules#resolver) to read from the file system the `.json` and `.puml` diagram files.
...
- The _Diagram Spec FS DAO_ reads the `json` file containing the diagram specs.
- The _Diagram PlantUML FS DAO_ reads the `puml` file containing the diagram PlantUML code.
